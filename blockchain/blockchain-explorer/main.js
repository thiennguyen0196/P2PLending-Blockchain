    ...
    if (txid && txid != '0' && channelName) {
        txModel.getTransactionByID(channelName, txid).then(row => {
            if (row) {
                ...
                let filter = rowString.replace(new RegExp("Asset:hcmus.fit.clc.lending.","g"),"");
                    filter = filter.replace(/\u0000/g,"");
                    filter = filter.replace(new RegExp("Asset:org.hyperledger.composer.","g"),"");
                    filter = filter.replace(new RegExp("org.hyperledger.composer.","g"),"");
                    filter = filter.replace(new RegExp("hcmus.fit.clc.lending.","g"),"");
                    filter = filter.replace(new RegExp("resource:","g"),"");
                    filter = filter.trim();

                row = JSON.parse(filter);
                ...
                const tmp = fcType.filter((item) => filter.includes(item))

                if (tmp.length > 0) {
                    row.fcType = tmp[0];
                } else {
                    row.fcType = 'sysTransaction';
                }
        ...
        statusMetrics.getPeerList(channelName, function (data) {
            ...
            if (data && data.length > 0) { 
                tmp = data.map((item, index) => { 
                    if (index === 0) { 
                        item.server_hostname = "peer0.org1.example.com"; 
                    } else { 
                        item.server_hostname = "peer0.org2.example.com"; 
                    } 
